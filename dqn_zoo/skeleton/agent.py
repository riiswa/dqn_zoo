...
class SkeletonDqn(parts.Agent):
  ...
  def __init__(
    ...
    def loss_fn(online_params, target_params, transitions, rng_key):
      ...
      q_tm1 = outputs.q_values
      action_probs = outputs.action_probs
      action_counts = jax.lax.stop_gradient(action_probs * self._frame_t)
      skeleton = action_counts[jnp.arange(self._batch_size), transitions.a_tm1] >= jnp.log(action_counts.max(axis=-1))**2
      ...
      skeleton_loss = -jnp.sum(jax.nn.one_hot(transitions.a_tm1, action_probs.shape[-1]) * jnp.log(action_probs), axis=-1)
      loss = (losses * skeleton).sum() / skeleton.sum() + skeleton_loss.mean()
      return loss
